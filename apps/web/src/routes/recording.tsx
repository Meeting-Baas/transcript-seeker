...
export default function MeetingPage() {
  ...
  const {
    ...
    botId ? ['meeting', botId, baasApiKey] : null,
    () => fetchMeeting(botId!),
    {
  ...
  useEffect(() => {
    if (!meeting || !baasApiKey || meeting.endedAt) return

    const updateInterval = setInterval(async () => {
      try {
        const updatedMeeting = await updateMeetingData(meeting, baasApiKey)
  ...
  if (error) {
    return <ErrorPage>Error loading meeting: {error.message}</ErrorPage>
  }

  if (!meeting) {
    return !botId ? <NotFoundPage /> : <FullSpinner />
  }

  return <Viewer botId={botId!} meetingId={meeting.id} />
}
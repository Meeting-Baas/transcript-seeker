...
Let's learn how to configure environment variables for Transcript Seeker. Transcript Seeker uses `dotenv-cli` to load the environment variables, making it easy for deployment. Transcript Seeker follows this structure for different environments:
...
These values are used by the api to perform google authentication. Please follow the [guide](/docs/concepts/google) for more details:
...
These values are used by the api to store user autehtncation data. Please follow the [guide](/docs/concepts/database) for more details:
